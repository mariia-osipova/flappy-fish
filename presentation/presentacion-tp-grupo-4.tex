...
\frametitle{Módulo game.py}
    ...
    Game, definido en \texttt{game.py}, es el marco general del que hereda la clase
    de juego \texttt{SwimFish}. Su constructor realiza toda la configuración inicial
    del entorno:

\begin{itemize}
    \item Inicializa Pygame, el temporizador y el objeto \texttt{Clock}; el objetivo de FPS se fija en 120.
    \item Crea la ventana principal del juego con dimensiones de 1000×600 píxeles.
    \item Carga los fotogramas del fondo animado desde \texttt{../data/img/fondo\_animado} y los configura para rotar a 30 FPS.
    \item Carga la música de fondo (\textit{Linkin Park fondo.ogg}) y el sonido del salto (\textit{efecto bubble.ogg}).
    \item Carga y escala el sprite de la tubería (\textit{alga2.png}) y genera su máscara de colisión pixel-perfect.
    \item Define el tamaño del hueco vertical entre tuberías (\texttt{hueco\_entre\_tuberias = 300}).
    \item Inicia un evento personalizado (\texttt{evento\_nueva\_tuberia}) que se activa cada 1500 ms para generar nuevos obstáculos.
\end{itemize}
